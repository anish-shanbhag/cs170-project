...
def validate_input(G: nx.Graph):
    ...
    for u, v, d in G.edges(data=True):
        assert (
            u != v
        ), "Edges should be between distinct vertices (a penguin is experiencing inner-conflict)"
        ...
        assert (
            d["weight"] <= MAX_WEIGHT
        ), f"Edge weights cannot be greater than {MAX_WEIGHT}"
    assert G.number_of_edges() <= MAX_EDGES, "Graph has too many edges"
    assert (
        sum(d for u, w, d in G.edges(data="weight")) >= MAX_WEIGHT * MAX_EDGES * 0.05
...
def visualize(G: nx.Graph):
    ...
    for k, v in partition.items():
        pos.update(
            nx.shell_layout(
                ...
                nlist=[v],
                center=(
                    circle_size * math.cos(math.tau * k / len(partition)),
    ...
    nx.draw_networkx_nodes(
        G, pos, node_color=[output[n] for n in G], cmap=cm.get_cmap("tab20b")
    )
    ...
    nx.draw_networkx_edges(
        ...
        edge_color=[x[2] for x in crossing_edges],
        edge_cmap=cm.get_cmap("Blues"),
        edge_vmax=max_weight * 1.5,
        edge_vmin=max_weight * -0.2,
    )
    nx.draw_networkx_edges(
        G,
        pos,
        ...
        edge_color=[x[2] for x in within_edges],
...
def run_all(solver, in_dir, out_dir, overwrite: bool = False):
    for file in tqdm([x for x in os.listdir(in_dir) if x.endswith(".in")]):
        run(
            solver,
            str(Path(in_dir) / file),
            str(Path(out_dir) / f"{file[:-len('.in')]}.out"),
            overwrite,
        )

...
for name in range(300):
    ...
    if os.path.isfile(f"outputs/{short_name}.out"):
        old_score = score(
            read_output(
                read_input(f"inputs/{short_name}.in"), f"outputs/{short_name}.out"
            )
        )
        best_score = scores[short_name]
        if ((old_score - best_score) / best_score) > 0.000001:
            short.append(name)
    ...
    if os.path.isfile(f"outputs/{medium_name}.out"):
        old_score = score(
            read_output(
                read_input(f"inputs/{medium_name}.in"), f"outputs/{medium_name}.out"
            )
        )
    ...
    if os.path.isfile(f"outputs/{long_name}.out"):
        old_score = score(
            read_output(
                read_input(f"inputs/{long_name}.in"), f"outputs/{long_name}.out"
            )
        )
...
def checkandreplace(short_name):
    stored = score(
        read_output(read_input(f"inputs/{short_name}.in"), f"outputs/{short_name}.out")
    )
